...
Example:
    python3 oliver/main_reduced_additional_diff.py
"""
...
feature_variant = "corr_compact_pruned_diff"
...
### 2.4 Additional weather and vegetation difference features
# Differences add short-term momentum without using total_cases or crossing city series.
weather_diff_plan = {
    "reanalysis_specific_humidity_g_per_kg": [1, 2, 4],
    "reanalysis_dew_point_temp_k": [1, 2, 4],
    "reanalysis_min_air_temp_k": [1, 2, 4],
    "precipitation_amt_mm": [1, 2, 4],
    "reanalysis_precip_amt_kg_per_m2": [1, 2, 4],
    "ndvi_sw": [10, 11, 14],
    "ndvi_nw": [10, 11, 14],
}

for feature, periods in weather_diff_plan.items():
    for period in periods:
        column = f"{feature}_diff_{period}"
        engineered_feature_data[column] = data.groupby("city", sort=False)[feature].diff(period)
        engineered_feature_columns.append(column)
...
for city in ["sj", "iq"]:
    ...
    if city == "sj":
        for feature in sj_weather_roots:
            ...
            for column in engineered_feature_columns:
                if (
                    column.startswith(f"{feature}_lag_")
                    or column.startswith(f"{feature}_rolling_")
                    or column.startswith(f"{feature}_diff_")
                ):
    ...
    if city == "iq":
        ...
        diff_suffixes = ["diff_1", "diff_2", "diff_4"]
        ndvi_diff_suffixes = ["diff_10", "diff_11", "diff_14"]
        ...
        for feature in iq_weather_roots:
            ...
            for column in engineered_feature_columns:
                ...
                if feature.startswith("ndvi_") and suffix in ndvi_suffixes:
                    weather_feature_columns.append(column)
                elif feature.startswith("ndvi_") and suffix in ndvi_diff_suffixes:
                    weather_feature_columns.append(column)
                elif "precip" in feature and suffix in precip_suffixes:
                    weather_feature_columns.append(column)
                elif "precip" in feature and suffix in precip_rolling_suffixes:
                    weather_feature_columns.append(column)
                elif "precip" in feature and suffix in diff_suffixes:
                    weather_feature_columns.append(column)
                    ...
                    weather_feature_columns.append(column)
                elif feature.startswith("reanalysis_") and suffix in diff_suffixes:
                    weather_feature_columns.append(column)

    selected_columns = seasonality_feature_columns + weather_feature_columns

    # Reduced city-specific feature set from the correlation-driven sandbox run.
    if city == "sj":
        selected_columns = seasonality_feature_columns + [
            column
            ...
            if (
                ...
                or (
                    (
                        column.startswith("station_avg_temp_c_lag_")
                        or column.startswith("station_min_temp_c_lag_")
                        or column.startswith("station_max_temp_c_lag_")
                    )
                    and int(column.rsplit("_", 1)[-1]) in [6, 7, 8, 9, 10, 11, 12]
                )
                or (
                    (
                        column.startswith("station_avg_temp_c_rolling_")
                        or column.startswith("station_min_temp_c_rolling_")
                        or column.startswith("station_max_temp_c_rolling_")
                    )
                    and int(column.split("_rolling_")[1].split("_")[0]) in [10, 12, 14]
                )
                or (
                    (
                        column.startswith("reanalysis_dew_point_temp_k_lag_")
                        or column.startswith("reanalysis_specific_humidity_g_per_kg_lag_")
                    )
                    and int(column.rsplit("_", 1)[-1]) in [2, 3, 5, 6, 8, 10, 12]
                )
                or (
                    (
                        column.startswith("reanalysis_dew_point_temp_k_rolling_")
                        or column.startswith("reanalysis_specific_humidity_g_per_kg_rolling_")
                    )
                    and int(column.split("_rolling_")[1].split("_")[0]) in [4, 6, 8, 10, 12, 14]
                )
                or (
                    (
                        column.startswith("reanalysis_dew_point_temp_k_diff_")
                        or column.startswith("reanalysis_min_air_temp_k_diff_")
                        or column.startswith("reanalysis_specific_humidity_g_per_kg_diff_")
                    )
                    and int(column.rsplit("_", 1)[-1]) in [1, 2, 4]
                )
            )
        ]
    ...
    if city == "iq":
        selected_columns = seasonality_feature_columns + [
            ...
            if (
                ...
                or (
                    (
                        column.startswith("reanalysis_specific_humidity_g_per_kg_lag_")
                        or column.startswith("reanalysis_dew_point_temp_k_lag_")
                        or column.startswith("reanalysis_min_air_temp_k_lag_")
                        or column.startswith("station_min_temp_c_lag_")
                    )
                    and int(column.rsplit("_", 1)[-1]) in [1, 2, 3, 5]
                )
                or (
                    (
                        column.startswith("reanalysis_specific_humidity_g_per_kg_rolling_")
                        or column.startswith("reanalysis_dew_point_temp_k_rolling_")
                        or column.startswith("reanalysis_min_air_temp_k_rolling_")
                        or column.startswith("station_min_temp_c_rolling_")
                    )
                    and int(column.split("_rolling_")[1].split("_")[0]) in [3, 4, 5, 6, 7, 8]
                )
                or (
                    (
                        column.startswith("ndvi_sw_lag_")
                        or column.startswith("ndvi_nw_lag_")
                    )
                    and int(column.rsplit("_", 1)[-1]) in [10, 11, 14]
                )
                or (
                    (
                        column.startswith("precipitation_amt_mm_lag_")
                        or column.startswith("reanalysis_sat_precip_amt_mm_lag_")
                    )
                    and int(column.rsplit("_", 1)[-1]) in [2, 3]
                )
                or (
                    (
                        column.startswith("precipitation_amt_mm_rolling_")
                        or column.startswith("reanalysis_sat_precip_amt_mm_rolling_")
                    )
                    and int(column.split("_rolling_")[1].split("_")[0]) in [4, 7]
                )
                or (
                    (
                        column.startswith("reanalysis_specific_humidity_g_per_kg_diff_")
                        or column.startswith("reanalysis_dew_point_temp_k_diff_")
                        or column.startswith("reanalysis_min_air_temp_k_diff_")
                    )
                    and int(column.rsplit("_", 1)[-1]) in [1, 2, 4]
                )
                or (
                    (
                        column.startswith("ndvi_sw_diff_")
                        or column.startswith("ndvi_nw_diff_")
                    )
                    and int(column.rsplit("_", 1)[-1]) in [10, 11, 14]
                )
                or (
                    (
                        column.startswith("precipitation_amt_mm_diff_")
                        or column.startswith("reanalysis_precip_amt_kg_per_m2_diff_")
                    )
                    and int(column.rsplit("_", 1)[-1]) in [1, 2, 4]
                )
            )
